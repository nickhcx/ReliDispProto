...
import numpy as np
from BaseStage.layers.network import embedding
from BaseStage.layers.network import encoder
from BaseStage.layers.CNNEncoder import cnnEncoder
from transformers import BertTokenizer, BertModel
# GPT-2, GPT-Neo require transformer==4.0
#from transformers import GPT2Tokenizer, GPT2Model, GPTNeoModel
...
class BERTSentenceEncoder(nn.Module):
    ...
    def __init__(self, pretrain_path, max_length, word_embedding_dim, cnn_hidden_size, pos_embedding_dim):
        ...
        # self.GPTNeo = GPTNeoModel.from_pretrained("EleutherAI/gpt-neo-125m")
    ...
    def forward(self, inputs):
        ...
        #word_embed = self.GPT2(inputs["word"], attention_mask=inputs["mask"]).last_hidden_state
        #word_embed = self.GPTNeo(inputs["word"], attention_mask=inputs["mask"]).last_hidden_state
    ...
    def tokenize(self, raw_tokens, pos_head, pos_tail):
        ...
        for token in raw_tokens:
            ...
            #tokens += self.GPT2tokenizer.tokenize(token)
            ...
            cur_pos += 1
        #indexed_tokens = self.GPT2tokenizer.convert_tokens_to_ids(tokens)
