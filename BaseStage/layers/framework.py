...
class BaseRCFramework:
    ...
    def train(self, model, bert_optim, learning_rate, train_iter, val_step, val_iter, save_ckpt, warmup_step=300, weight_decay=1e-5,
        ...
        for it in range(start_iter, start_iter+train_iter):
            ...
            print('step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%'.format(it + 1, loss,
                                                                           100 * acc) + '\r')
    ...
    def eval(self, model, val_iter, ckpt=None):
        ...
        with torch.no_grad():
            for it in range(val_iter):
                ...
                sys.stdout.flush()
                print('[EVAL] step: {0:4} | accuracy: {1:3.2f}%'.format(it + 1, 100 * iter_overall_acc / iter_sample) + '\r')
